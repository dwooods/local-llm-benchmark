...
benchmarks/                  promptfoo suites — run these to reproduce a scored suite
  ...
  pi-crash-capture/          1Hz thermal/power logger (pi_thermal_log.py) and the CSV it
                             captured across an actual Pi hard reset (thermal.csv) — see
                             "Pi setup" below and FINDINGS.md
scripts/
...
**not** eliminate it. A later instrumented reproduction (`benchmarks/pi-crash-capture/` — see
below) identified the actual mechanism: it's not a fixed temperature trip point, it's sustained
per-model power draw against the cooler's real ceiling, and per-model power draw isn't published
anywhere — see FINDINGS.md for the full data. Don't run an unattended multi-hour suite on the Pi
assuming a clean finish — checkpoint intermediate results (e.g. `--output` to a file per provider,
or rely on promptfoo's own cache) so a crash mid-suite doesn't lose already-completed test cases.
...
`pi-crash-capture/pi_thermal_log.py` is different — it's a plain, runnable logger, not a
reproduction of the watchdog. It samples `vcgencmd measure_temp`, the throttle flags, every PMIC
rail, CPU clock, and fan RPM once a second and `fsync`s every row to disk, because Raspberry Pi OS
keeps its journal in RAM by default — without a logger fsync'ing to disk, nothing about a crash
moment survives the hard reset that follows it (confirmed here: `journalctl -b -1` after a reset
returned "no persistent journal was found"). `pi-crash-capture/thermal.csv` is the actual data it
captured across a real crash, reset boundary included. Run it yourself with:

```bash
python3 pi-crash-capture/pi_thermal_log.py --out thermal.csv
```

before starting a sustained suite in a separate terminal, and see FINDINGS.md for what the
captured run showed.
